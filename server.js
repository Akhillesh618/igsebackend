...
app.post("/register", async (req, res) => {
  try {
    ...
    if (
      ...
      const existingUserVoucher = await user.find({
      ...
      if (existingUser.length == 0 && existingUserVoucher.length == 0) {
        ...
        collection
          .find({ voucherCode: req.body.voucherCode })
...
app.post("/submitbill", async (req, res) => {
  try {
    ...
    if (
      ...
      !req.body.electricity_reading_Day ||
      !req.body.electricity_reading_Night ||
      !req.body.gas_reading
    ) {
...
app.get("/propertycount", async function (req, res) {
  const collection = client.db("igse").collection("users");
  ...
  collection.find({}).toArray((err, documents) => {
    ...
    for (const doc of documents) {
      allproperty[doc.propertyType]+=1
    }
